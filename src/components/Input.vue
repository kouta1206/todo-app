    ...
    <transition name="slide-up-button">
      <div class="create-todo-fields" v-show="createTodoFields">
        <form class="create-txt-todo" @submit.prevent="addTodo()" v-cloak>
          <input
            class="create-txt-fields"
            type="text"
            placeholder="新しいタスク"
            ref="focusThis"
            v-model.trim="createTodoItem"
          />
          <input
            class="submit-btn"
            type="submit"
            value="保存"
            @click="createTodoFields = !createTodoFields"
          />
        </form>
      </div>
    </transition>
...
.slide-up-button-enter,
.slide-up-trigger-leave-to {
